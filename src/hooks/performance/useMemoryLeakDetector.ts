...
export const useMemoryLeakDetector = (options: LeakDetectionOptions = {}) => {
  ...
  const checkForLeaks = useCallback((): LeakInfo => {
    const now = Date.now();
    const aliveObjects = objectsRef.current.filter(ref => ref.deref() !== undefined);
    const retainedCount = aliveObjects.length;
