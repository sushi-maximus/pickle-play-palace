...
export const useWizardValidation = () => {
  const validateWizardStep = (currentStep: number, formData: EventFormData): boolean => {
    switch (currentStep) {
      case 1:
      ...
      case 3:
        ...
        // For wizard (creating events): Always require future date/time
  ...
  const getWizardValidationErrors = (currentStep: number, formData: EventFormData) => {
    ...
    if (currentStep === 3) {
      ...
      if (!formData.eventDate) {
        ...
        // For wizard: Always require future date/time
        const eventDateTime = new Date(`${formData.eventDate}T${formData.eventTime}`);
        const currentTime = new Date();
  ...
  return { validateWizardStep, getWizardValidationErrors };
};
